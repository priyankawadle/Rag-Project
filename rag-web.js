...
const webPageSplits = await textSplitter.splitDocuments(documents); // Split document into smaller chunks

// Step 3: Embed and Store in a Vector Database
const embedder = new OpenAIEmbeddings(); // Initialize OpenAI embeddings model
const store = await MemoryVectorStore.fromDocuments(webPageSplits, embedder); // Store embedded chunks in memory

// Step 4: Retrieve Relevant Documents Based on a Query
const webPageRetrieval = store.asRetriever(); // Convert vector store into a retriever
const relevantDocs = await webPageRetrieval.invoke("How does GraphQL work"); // Retrieve relevant documents based on query
...
const ragChain = await createRetrievalChain({
  retriever: webPageRetrieval, // Use the document retriever
